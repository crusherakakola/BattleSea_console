...
class BoardNearException(BoardException):
    def __str__(self):
        return '\nНельзя ставить корабль рядом с другим кораблем!\n'
...
class CoordinateOutException(BoardException):
    def __str__(self):
...
class ShotUsedException(BoardException):
    def __str__(self):
...
class Board:
    def __init__(self, hidden=False):
        self.field = [[BoardDot(hidden) for j in range(6)] for i in range(6)]
...
class Ai(Player):
    def ask(self):
        c = Coordinates(randint(1, 6), randint(1, 6))
        print(f'\nКомпьютер сделал ход: {c}')
        return c
...
class Game:
    def __init__(self):
        ...
        ai_board = self.random_board(hidden=True)  # Поменять видимость поля компьютера
    ...
    def random_board(self, hidden=False):
        ...
        while index <= 6:
            if attempts == 2000 or board.ships_count() > 10:
                ...
                attempts = 0
            try:
                ship = Ship(length_list[index], Coordinates(randint(1, 6), randint(1, 6)), randint(1, 2))
                board.add_ship(ship)
            except BoardException:
                pass
            else:
    ...
    def greet(self):
        ...
        print('--------------------')
        print('Координаты необходимо вводить в одну строку через пробел: "x y"')
        print('x - номер строки')
        print('y - номер колонки\n')
        n = input('Для старта игры нажмите Enter!')
        n = 0
